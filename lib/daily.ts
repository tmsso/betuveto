 ...
 *  daily puzzle (any combo counts). The current run may end **today or yesterday** — so a
 *  player who hasn't played yet today still sees their real streak rather than 0 — and a
 *  gap of >= 2 days breaks it. */
async function computeStreak(sql: Sql, playerId: string): Promise<StreakInfo> {
